...
- 使用 16 字节盐值与 10000 次迭代，有效抵御彩虹表与暴力破解；
- 哈希输出包含盐与哈希拼接后的字节数组，经 Base64 编码存储；
- 验证流程通过“逐字节常量时间比较”避免时序攻击；
- 异常捕获机制防止敏感信息泄露；
...
- [src/application/service/PasswordHashService.cs](file://src/application/service/PasswordHashService.cs#L1-L53)
...
## 核心组件
- PasswordHashService：提供密码哈希与验证功能，内部使用 PBKDF2（Rfc2898DeriveBytes）+ SHA256，固定盐长与迭代次数，输出 Base64 字符串。
...
Ctrl->>PHS : VerifyPassword(明文密码, Base64字符串)
PHS-->>Ctrl : true/false
alt 验证通过
Ctrl->>JWT : 生成 JWT
JWT-->>Ctrl : token
...
- [src/application/controllers/auth/AuthController.cs](file://src/application/controllers/auth/AuthController.cs#L86-L119)
- [src/application/controllers/auth/AuthController.cs](file://src/application/controllers/auth/AuthController.cs#L147-L168)
- [src/application/service/PasswordHashService.cs](file://src/application/service/PasswordHashService.cs#L12-L25)
- [src/application/service/PasswordHashService.cs](file://src/application/service/PasswordHashService.cs#L27-L52)
- [src/data/entities/User.cs](file://src/data/entities/User.cs#L27-L33)
...
- 算法与参数
  - PBKDF2（Rfc2898DeriveBytes）+ SHA256，盐长 16 字节，迭代 10000 次。
  - 该组合具备强抗彩虹表能力，且迭代次数可按硬件能力提升以增强安全性。
- 哈希生成流程（HashPassword）
...
GenSalt --> Derive["PBKDF2(SHA256)<br/>迭代10000次"]
...
### 注册流程中的安全要点
- 注册密钥校验通过后，使用 PasswordHashService 对明文密码进行哈希；
...
- 调用 PasswordHashService 进行验证，若失败统一返回 401；
- 成功后生成 JWT 并返回给客户端。
...
### 时序攻击防护与异常处理
- 验证过程采用逐字节比较，避免因早期不同而提前返回，从而降低时序攻击风险；
...
- AuthController 在注册与登录中分别调用哈希与验证逻辑；
...
## 性能考量
- 迭代次数与安全权衡：当前设置为 10000 次，可在通用硬件上提供良好安全与性能平衡。若服务器资源充足，可逐步提高迭代次数以增强抗暴力破解能力；反之则应保持稳定，避免影响用户体验。
- 建议：
...
- 登录失败
  ...
  - 若出现异常，VerifyPassword 会捕获并返回 false，需检查日志定位具体原因。
- 日志与异常
...
## 结论
PasswordHashService 通过 PBKDF2（Rfc2898DeriveBytes）+ SHA256 的标准化密码哈希方案，结合固定盐长与迭代次数，有效提升了抗彩虹表与暴力破解能力；其“盐+哈希”的拼接与 Base64 编码存储方式，确保数据库中不保存明文密码；验证流程采用逐字节常量时间比较与异常捕获，进一步增强了安全性与健壮性。在注册与登录流程中，服务被正确注入并使用，实现了密码安全存储与验证。建议在保证向后兼容的前提下，按需调整迭代次数以平衡安全与性能。

## 附录
- 关键实现位置参考
  - 哈希生成：[HashPassword 方法](file://src/application/service/PasswordHashService.cs#L12-L25)
  - 验证流程：[VerifyPassword 方法](file://src/application/service/PasswordHashService.cs#L27-L52)
